...
const ProfileTab = ({ onComplete }) => {
  ...
  const setdataFn = async () => {
    try {
      const response = await axios.get(`http://13.127.222.190/api/category/getData/${id}`);
  ...
  const uploadImage = async (imageFiles, id) => {
    ...
    try {
      const response = await axios.post(
        `http://13.127.222.190/api/category/updateImage/${id}`,
        formData,
        {
          headers: {
    ...
    try {
      ...
      if (id) {
        ...
        response = await axios.put(
          `http://13.127.222.190/api/category/UpdateCategory/${id}`,
          payload,
          {
        ...
        response = await axios.post(
          'http://13.127.222.190/api/category/createCategory',
          payload,
          {
            headers: { 'Content-Type': 'application/json' },
          }
